...
public class Treap<K extends Comparable> {
  ...
  (on left side) or x (on right side)
    ...
    public static class Node<K extends Comparable> {
        ...
        public Node<K>[] split(K split) {
            Node<K> tmp = null;

            Node<K>[] res = (Node<K>[]) Array.newInstance(this.getClass(), 2);

            if (this.key.compareTo(split) <= 0) {
                if (this.right == null) {
                    res[1] = null;
                } else {
                    Node<K>[] rightSplit = this.right.split(split);
                    res[1] = rightSplit[1];
                    tmp = rightSplit[0];
                }
                ...
                if (left == null) {
                    ...
                    Node<K>[] leftSplit = this.left.split(split);
                    res[0] = leftSplit[0];
                    tmp = leftSplit[1];
                }
        ...
        @Override
        public String toString() {
            return String.format("(%s,%d,%d[%d]m%d s%d M%d +%d)", key.toString(), priority, valueOf(this), size, statistic.minValue, statistic.sumValue, statistic.maxValue, addPromise);
        }
